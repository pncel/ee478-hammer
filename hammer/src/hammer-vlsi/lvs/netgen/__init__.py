...
class Netgen(HammerLVSTool, TCLTool):
    ...
    def run_netgen(self) -> bool:
        ...
        if bool(self.get_setting("lvs.netgen.generate_only")):
            ...
            # Temporarily disable tag to make run output more readable
            HammerVLSILogging.enable_tag = False
            self.run_executable(args, cwd=self.run_dir)
            self.report_netgen_output()
            HammerVLSILogging.enable_tag = True

        return True

    def report_netgen_output(self) -> bool:
        lvs_log_success_string = b'Circuits match uniquely.' # Check the log for this string (must be bytes type)
        log_fname =  os.path.join(self.run_dir, self.top_module + ".lvs.log")
        with open(log_fname, 'rb') as f:
           stream = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) # Use mem-map to effeciently search file
           if stream.find(lvs_log_success_string) == -1: # If success string not found...
               self.logger.warning(self.lvs_fail_output)
           else:
               self.logger.info(self.lvs_success_output)
