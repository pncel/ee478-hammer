...
class Genus(HammerSynthesisTool, CadenceTool):
    ...
    def init_environment(self) -> bool:
        ...
        # Load input files and check that they are all valid HDL files (Verilog/SystemVerilog or VHDL).
        if not self.check_input_files([".v", ".sv", ".svh" ,".vh", ".vi", ".vhd", ".vhdl"]):
            return False
        ...
        # Add any verilog_synth wrappers (which are needed in some technologies e.g. for SRAMs) which need to be synthesized.
        ...
        if verilog_sv_files:
            verbose_append("read_hdl -sv {{ {} }}".format(" ".join(verilog_sv_files)))
